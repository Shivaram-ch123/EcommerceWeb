...
<body>
	...
	<c:forEach var="entry" items="${ordersList}">
		<div
			style="border: 1px solid black; margin: 10px; padding: 10px; position: relative;">
			...
			<c:forEach var="item" items="${entry.key.products}">
				<li
					style="position: relative; padding: 8px 0; border-bottom: 1px solid #ccc;">
					${item.product.name} - Quantity: ${item.quantity} <!-- Cancel Button -->
